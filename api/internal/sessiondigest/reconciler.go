...
// Reconciler discovers immutable, fully indexed content slices and creates one
// deterministic digest build job for each versioned source identity. It never
// runs in the upload transaction, so digest work cannot delay or roll back an
// accepted Session upload.
...
func (r *Reconciler) Start(ctx context.Context) {
	go func() {
		if _, err := r.RunOnce(ctx); err != nil {
			log.Printf("session digest reconciler failed: %v", err)
		}
...
func (r *Reconciler) RunOnce(ctx context.Context) (int, error) {
	...
	rows, err := tx.QueryContext(ctx, `
		...
		LIMIT $3
		FOR UPDATE OF sl SKIP LOCKED`, r.config.DigestVersion, r.config.RedactionVersion, r.config.ReconcileBatch)
	if err != nil {
		return 0, err
	}
	...
	for rows.Next() {
		var item candidate
		if err := rows.Scan(&item.sliceID, &item.sessionID, &item.revisionID, &item.generationID, &item.contentEpoch); err != nil {
			rows.Close()
			return 0, err
		}
	...
	for _, item := range candidates {
		...
		err := tx.QueryRowContext(ctx, `
			...
			RETURNING id::text`, item.sliceID, item.revisionID, item.generationID,
			item.contentEpoch, r.config.DigestVersion, r.config.RedactionVersion).Scan(&digestRevisionID)
		...
		if _, err := tx.ExecContext(ctx, `
			...
			DO NOTHING`, JobType, item.sessionID, item.generationID, digestRevisionID, item.contentEpoch); err != nil {
			return 0, err
		}
		created++
	}
