...
func (p *Processor) Process(ctx context.Context, job sessionsync.ProcessingJob) error {
	...
	rows, err := p.db.QueryContext(ctx, `
		SELECT source_start_cursor, source_end_cursor, event_type,
			''::text, ''::text,
			CASE event_type
			...
			WHEN 'user' THEN jsonb_build_object('message', jsonb_build_object('content', COALESCE((
				...
				FROM (
					SELECT ordinal, CASE block->>'type'
						WHEN 'text' THEN jsonb_build_object('type', 'text', 'text', left(COALESCE(block->>'text', ''), 8192))
						WHEN 'tool_result' THEN jsonb_build_object(
							'type', 'tool_result', 'tool_use_id', COALESCE(block->>'tool_use_id', ''),
							'content', left(COALESCE(block->>'content', ''), 512) || ' ' || right(COALESCE(block->>'content', ''), 2048))
				...
				FROM (
					SELECT ordinal, CASE block->>'type'
						...
						WHEN 'tool_use' THEN jsonb_build_object(
							'type', 'tool_use', 'id', COALESCE(block->>'id', ''), 'name', COALESCE(block->>'name', ''),
			...
			ELSE NULL END,
			content_sha256, octet_length(COALESCE(content_payload, '{}'::jsonb)::text)
		FROM session_content_events
		WHERE content_projection_revision_id = $1
			AND source_start_cursor >= $2 AND source_end_cursor <= $3
		ORDER BY source_start_cursor, source_end_cursor, id`, target.Revision.ProjectionRevisionID,
		target.Revision.StartCursor, target.Revision.EndCursor)
	if err != nil {
		return p.recordFailure(ctx, job, err)
	}
	for rows.Next() {
		var event Event
		var payload []byte
		if err := rows.Scan(&event.StartCursor, &event.EndCursor, &event.EventType,
			&event.Summary, &event.Excerpt, &payload, &event.ContentSHA, &event.PayloadBytes); err != nil {
			rows.Close()
			return p.recordFailure(ctx, job, err)
		}
