...
func NewWorker(queue JobQueue, processor JobProcessor, owner string) (*Worker, error) {
	if queue == nil || processor == nil || owner == "" {
		return nil, errors.New("digest job queue, processor, and owner are required")
	}
	return &Worker{
...
func (w *Worker) Start(ctx context.Context) {
	go func() {
		if err := w.RunOnce(ctx, time.Now().UTC()); err != nil {
			log.Printf("session digest worker failed: %v", err)
		}
...
func (w *Worker) RunOnce(ctx context.Context, now time.Time) error {
	jobs, err := w.queue.ClaimTypes(ctx, w.owner, now, w.leaseTTL, w.batchLimit, []string{JobType})
	if err != nil {
		return err
	}
	...
	for _, job := range jobs {
		...
		if processErr == nil || errors.Is(processErr, ErrStaleDigestSource) {
			...
			if completeErr != nil && firstError == nil {
				...
				firstError = fmt.Errorf("digest job %s lost its lease before completion", job.ID)
			}
			continue
		}
		preserveAttempt := errors.Is(processErr, ErrDigestStatePersistence)
		ok, failErr := w.queue.Fail(ctx, job.ID, w.owner, finishedAt,
			digestRetryDelay(job.Attempts), preserveAttempt, FailureCode(processErr))
		if failErr != nil && firstError == nil {
			firstError = failErr
		} else if !ok && firstError == nil {
			firstError = fmt.Errorf("digest job %s lost its lease before failure update", job.ID)
		}
	}
	return firstError
}
	...
	for {
		...
		case now := <-ticker.C:
			...
			if err != nil || !ok {
				...
				return fmt.Errorf("digest job %s lost its lease during processing", job.ID)
			}
		case <-ctx.Done():
			cancel()
			<-result
