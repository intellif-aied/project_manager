...
// TestProjectSafeEventMatchesPostgresLegacySQL keeps the MinIO reader migration tied to
// the exact PostgreSQL projection it replaces. It is intentionally integration-only.
func TestProjectSafeEventMatchesPostgresLegacySQL(t *testing.T) {
	...
	longOutput := strings.Repeat("head", 500) + strings.Repeat("tail", 1500)
	tests := []struct {
		eventType string
		payload   any
	}{
