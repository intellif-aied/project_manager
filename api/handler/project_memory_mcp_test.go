...
func TestProjectMemoryMCPReadsBoundJobContext(t *testing.T) {
	...
	mock.ExpectQuery("SELECT input_json").
		WithArgs("305", testMemoryFingerprint).
...
func TestProjectMemoryMCPValidatesAndWritesBoundProposal(t *testing.T) {
	...
	input, err := json.Marshal(reportmemory.ConsolidationInput{
		...
		CurrentThemes: []reportmemory.InputTheme{{ThemeRef: "theme-001", EvidenceRef: "evidence-1", Title: "IF-Knowledge"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	mock.ExpectExec("UPDATE report_project_memory_jobs SET").
		WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg(), "305", testMemoryFingerprint).
		WillReturnResult(sqlmock.NewResult(0, 1))
	args := json.RawMessage(`{"proposal_json":{"schema_version":"project-memory-maintenance/v2","operations":[{"operation_id":"op-1","operation":"create_project","theme_ref":"theme-001","evidence_refs":["evidence-1"],"temp_ref":"new-1","canonical_name":"IF-Knowledge","confidence":0.9}]}}`)
