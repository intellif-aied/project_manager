...
func defaultReportAgentInstructions(credentialSlot string) string {
	return strings.Join([]string{
		...
		"忠实合并规则：同一主线出现按时间递进的多个结果或多个版本时，必须以最后完成的 highlight 表达当前状态，前序阶段只能合并其中，不得用旧版本覆盖新结果。status=partial 本身不能机械生成待跟进；只有按时间合并后仍未完成或存在具体 unresolved 才能保留，后续已完成的旧 partial 必须并入最终成果。严禁猜测环境级别；测试、预发、目标或生产环境只能按来源明确证据表述，未明确时省略环境，不得写成生产。",
...
func defaultReportAgentStartPromptTemplate(credentialSlot string) string {
	return strings.Join([]string{
		...
		"同一主线有多个递进结果时，以最后完成的 highlight 为当前状态，禁止用旧版本覆盖新结果。status=partial 不能直接生成待跟进，合并后仍未完成或存在具体 unresolved 时才能保留。环境级别必须有来源明确证据；不得把测试、预发、目标或未指定环境改写为生产。",
...
func buildReportRunMessage(startPromptValues map[string]string, message string, credentialSlot string) string {
	...
	if selectionID := strings.TrimSpace(startPromptValues["report_source_selection_id"]); selectionID != "" {
		parts = append(parts,
			...
			"忠实合并规则：同一主线有多个按时间递进的结果或版本时，以最后完成的 highlight 表达当前状态，前序阶段只能合并其中，不得用旧版本覆盖新结果。status=partial 不能直接生成待跟进；合并后仍未完成或存在具体 unresolved 时才能保留，后续已完成的旧 partial 必须并入最终成果。严禁猜测环境级别；测试、预发、目标或生产环境只能按来源明确证据表述，未明确时省略环境，不得写成生产。",
		)
	}
	message = strings.TrimSpace(message)
	if message != "" {
