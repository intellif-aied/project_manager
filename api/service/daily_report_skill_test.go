...
func TestReportSkillMarkdownUsesReportBriefForPersonalDaily(t *testing.T) {
	...
	required := []string{
		...
		"Keep one or two reader-worthy deliverables per workstream",
		"use a third only when removing it would lose an independent necessary outcome",
		"35–90 Chinese characters",
		"representative non-duplicate fact_refs",
		"references prove an outcome, not coverage",
		"Keep one to three workstreams and roughly two to six reader-facing lines",
		"A busy Session, many Facts",
		"repositories, CWDs, branches, files, models, stages",
