...
8. Produce concise Chinese Markdown suitable for the selected report_type.
   - Before writing a personal report, silently rewrite every highlight into a user-facing outcome: subject + delivered capability/current state. Remove implementation process, commands, files, paths, test evidence, version history, environment details, and proof clauses from that rewrite.
   - Never omit a material outcome merely because it contains noisy implementation evidence. Map pure proof-only highlights to the capability they verify instead of creating a separate proof item. Merge genuine duplicates, but keep design, implementation, deployment, quality improvement, failure/blocker, and unresolved follow-up when they carry different meaning.
...
- For personal reports, use as many items as needed to cover every materially distinct outcome; there is no fixed 3-to-5 or maximum-6 limit. Prefer the exact structure 今日完成 / 进行中与待跟进, with an optional 重要决定 section only when it adds user-facing value. Omit empty sections.
- For a personal daily report, read the complete canonical selection-level highlight list and cover every meaningful completed highlight either directly or through an explicit same-initiative merge. Do not arbitrarily drop a completed highlight or replace it with lower-value process context.
- For digest_v2 personal daily reports, create at least one top-level numbered or bulleted outcome for every canonical highlight. The number of top-level outcome items must be at least outcome_coverage.source_count. Rewrite proof-only highlights into the user-facing capability they establish instead of deleting them. This is a completeness rule, not an instruction to print the count; never expose the count in the report.
- Create 进行中与待跟进 only after chronological merging, and only when the current state still remains partial, blocked, failed, or has a concrete unresolved item. A historical/intermediate partial highlight is not automatically a follow-up: if it has no concrete unresolved item or a later highlight completes the same initiative, absorb it into the final outcome instead of inventing stale follow-up work. Otherwise omit the section entirely; never write “无”, “暂无”, or an equivalent empty placeholder.
- Before write_report_result for a personal daily report, perform a silent coverage check keyed by every canonical highlight work_unit_ref. Map each ref to a report item or a clearly related merged initiative, then add concrete partial/blocked/unresolved items separately. If any materially distinct ref is missing, revise the draft before writing. Never expose work_unit_ref itself.
...
- Never rewrite a test/staging/target environment as production. If the source does not explicitly establish the environment tier, omit the tier instead of guessing it.
