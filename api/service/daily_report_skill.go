...
- Prefer sustained objectives, concrete deliverables, material state changes, blockers, and risks. Text length, technical detail, Session duration, tests, deployments, repositories, and Subagent output do not establish importance by themselves.
- Merge candidates with the same reader-facing objective. Attach implementation, documentation, validation, and fixes to the primary result through fact_refs instead of creating extra headings.
