...
- Group Facts into a two-level map. Start from one main Workstream and add another only when current Facts prove a distinct project or independently reviewable goal. Create Workstreams only from the first level: stable projects, products, or capabilities; keep models, modules, stages, datasets, and activities below.
...
- Before submitting, compare every subject pair. Subjects serving one daily objective or sharing one named entity, training pipeline, data pipeline, product, or capability must merge; evaluation, training, research, documentation, model variants, stages, and infrastructure are child outcomes unless current Facts prove separate goals.
...
- Keep one or two reader-worthy deliverables per workstream; use a third only when removing it would lose an independent necessary outcome. Each result should normally be 35–90 Chinese characters and combine closely related progress instead of listing every metric or operation. Cite only one to three representative non-duplicate fact_refs; references prove an outcome, not coverage.
- Keep one to three workstreams and roughly two to six reader-facing lines for a normal day; exceed three Workstreams only when current Facts explicitly name more independent projects. A busy Session, many Facts, repositories, CWDs, branches, files, models, stages, artifacts, tool calls, durations, or details never justify more Workstreams.
