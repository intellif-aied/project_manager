...
export function MarkdownEditor({
  ...
  const imageURLs = useMemo(
    () => [...value.matchAll(/!\[[^\]]*\]\((\/api\/v1\/report-images\/[^)\s]+)\)/g)].map((match) => match[1]),
    [value]
  );
  ...
  return (
    ...
    >
      ...
      />
      {uploadingImage || imageURLs.length ? (
        <div className="markdown-editor-images" aria-live="polite">
          {uploadingImage ? <div className="markdown-editor-image is-uploading">图片上传中…</div> : null}
          {imageURLs.map((url) => <img key={url} className="markdown-editor-image" src={url} alt="正文图片预览" />)}
        </div>
      ) : null}
    </div>
  );
