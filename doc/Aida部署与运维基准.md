...
单次发布的具体值填写在 `doc/v2/发布事项/YYYYMMDD-*.md`，必须以 [完整生产发布清单模板](v2/发布事项/完整生产发布清单模板.md) 为基础；本文只规定顺序：

1. 先生成完整发布清单，按 API/后台、Report Skill、MCP/默认 Agent、数据库迁移、历史数据、数据清理、Web、CLI、MinIO、Digest/报告链路和文档配置逐项标记“发布/仅核对/不涉及/阻断”。不得把最后一个提交、最后一个 Bug 或最后一次对话修改当成本次完整发布范围。
...
发布清单的所有阻断项必须为 0，所有发布项必须有实现、测试、数据或部署证据。任一项未确认，禁止生产发布。API-only 或 Web-only 仍必须填写完整清单并核对未发布组件的兼容性；“只发布一个服务”不等于可以省略其他范围。
...
- 发布清单最终状态必须明确写为“已发布”“未发布/阻断”或“回滚”，禁止用“部分完成”替代发布判定。
