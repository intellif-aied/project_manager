...
> Review 日期：2026-07-17；范围：本目录全部方案文档。以下是当时的历史 Review 结论，不是生产发布批准单。
> 结论：R1 已完成；R5A/R5B 获得开发授权并完成隔离环境首轮实现。完整生产发布仍必须以 [完整发布清单](./09-完整发布清单.md) 为唯一门禁。

> 事故修正：2026-07-18 实际发生了 R5B/API/Web/Skill 的不完整生产发布。该发布没有完成 R2/R3/R4 的 MinIO Reader、停止 Payload 写入和历史 JSONB 清理，因此不得被解释为本专项完整版本已通过。
...
- 当时 Review 结论：生产迁移/发布/清理未授权；后续实际发生的不完整发布已在 [完整发布清单](./09-完整发布清单.md) 登记为事故，不改变本 Review 的门禁结论。
