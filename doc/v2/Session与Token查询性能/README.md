...
开发唯一入口：[当前开发任务清单](./00-当前开发任务清单.md)。

```text
当前阶段：DEV-01 固化消费者清单
紧接任务：DEV-02 实现统一 MinIO Content Reader
允许开发：DEV-01、DEV-02
禁止执行：停止 Payload 写入、历史清理、生产发布
```

每次开发状态只按 DEV 编号更新；没有 DEV 编号、修改文件和验收结果的内容不算进度。
...
- 当前只允许在 `fea.0.0.1` 开发 DEV-01/DEV-02；157 用于后续功能验证，不把测试数据结论直接继承到生产；
