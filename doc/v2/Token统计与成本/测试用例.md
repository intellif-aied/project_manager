...
| 编号 | 场景 | 预期 |
|---|---|---|
| M01 | 空数据库执行全部迁移 | 只登记到 017，所有新表、约束和索引存在 |
| M02 | 生产数据副本从 016 升级 | 用户、Session、旧 Token、日报周报数量不变，升级原子成功 |
| M03 | 已登记旧 017-025 的测试数据库启动 | 不重复执行 017，不报错 |
| M04 | 回填工具不带 `--apply` | 只输出候选与缺失原始日志统计，不写数据库和对象存储 |
| M05 | 单 Session 分批回填并重跑 | 正式 Chunk、Parser、Revision 完成；重跑候选为 0，Token 不重复 |
| M06 | `--apply` 未提供正数 `--limit` | 拒绝执行，不产生批量任务 |
| M07 | 原始日志缺失或 JSONL 尾部损坏 | 记录失败清单，不伪造新 Usage Component |
