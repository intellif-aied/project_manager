...
- 同一 Source 的 generation/revision 原子切换可以转移 claim。
- 不同 Source 已持有 claim 时，只有稳定 Provider ID 与当前 Observation 的 `raw_usage_hash` 都完全一致，才把该事实视为跨 Source 重复：已有 Source 保持 claim，新 Source 只停用该事件的 active Component/Contribution，并继续保留自身独有事件。共享事实只计一次。
- 不同 Source 的相同 Provider ID 若 `raw_usage_hash` 不同，staging revision 必须标记 conflict；禁止任选一个值、逐字段取大或静默覆盖。
...
跨 Source claim 判定在 Revision 激活事务中按用户串行执行，避免两个 Source 并发激活时同时认为自己拥有同一 Provider 事实。精确重复只抑制重复 Contribution；内容不同仍进入 conflict，不能借跨 Source 去重绕过 Provider Parser 的质量门禁。
...
本期数据库结构统一由 `017_session_upload_report_source_and_token_analytics.sql` 一次升级完成。该文件按依赖顺序包含 Session 增量上传、内容投影、Usage 账本、报告来源、组织历史、价格成本和内容切片结构。生产从 `016` 直接升级到 `017`；不得再恢复拆分的 `018-025`，也不得在生产执行 `017` 后改写其内容。迁移 SQL 与 `schema_migrations` 版本登记必须处于同一事务，并通过 PostgreSQL advisory lock 防止多实例并发执行。

旧 Token 历史不直接写入新账本。上线前使用镜像内 `/usr/local/bin/migrate-session-ledger` 处理：

1. 不带 `--apply` 运行预览，确认 `candidates`、`missing_raw_sessions` 和 `missing_raw_total_tokens`。
2. 有原始日志的 Session 按不超过 4 MiB、500 条事件的完整 JSONL Chunk 进入正式 Prepare、Chunk、Finalize、Parser 和 Revision 流程。
3. `--apply` 必须同时提供正数 `--limit`，分批执行并等待 Content/Usage Job 清空后再继续下一批。
4. 无原始日志的 Session 不伪造 Usage Component，不参与新账本精确统计，必须列入上线差异报告。
5. 每批核对旧 `token_usage`、新 active Usage Component、Session 日聚合及质量状态；旧值只用于差异定位，不作为正确性基准。

发布前必须验证空数据库初始化、真实生产数据副本 `016 -> 017`、已执行旧 `017-025` 的测试数据库启动三条路径。
...
当前代码已完成阶段 3 实现：统一 migration 017、组织区间触发器、价格/汇率修正、成本 supersede、重算审计、物化查询快照、权限 API 和前端页面均已落地。空库集成测试已证明调组前后历史归属、总监/TL 权限、decimal 成本、重计价后旧查询快照稳定，以及尚无 component 的 pending Source 不会被漏报。当前无用户级分析开关；尚未完成生产旧数据全量回填和最终部署验收。
