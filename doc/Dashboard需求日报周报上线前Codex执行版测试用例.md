...
Codex 执行时不要手写 token。必须从 `doc/测试账号文档.md` 读取。
...
```bash
ssh 157 'cd /home/intellif/dev/project_manager && test -f doc/测试账号文档.md && test -f doc/Dashboard需求日报周报上线前全量测试用例.md'
...
```text
doc/test-reports/dashboard-requirements-reports-<PREFIX>.md
```
...
```text
doc/test-reports/<PREFIX>-ui.md
doc/test-reports/<PREFIX>-screenshots/
```
...
```text
doc/test-reports/dashboard-requirements-reports-summary-<YYYYMMDD>.md
```
...
```text
按 doc/Dashboard需求日报周报上线前Codex执行版测试用例.md 跑一轮上线前回归。
